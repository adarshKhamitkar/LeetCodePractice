...
from (      
    select p.product_name
        ...
        ,o.order_id
        ,o.order_date
        ,dense_rank() over (partition by product_name,product_id order by order_date desc) as rnk
    from Products p join Orders o on p.product_id = o.product_id
    group by p.product_name,p.product_id,order_id
    order by p.product_name,p.product_id,o.order_id,o.order_date desc
)tab
where tab.rnk=1
