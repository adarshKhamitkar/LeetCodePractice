...
*/
with frequent_orders as (
select customer_id,product_id
from(
 select *,
        max(most_frequent) over (partition by customer_id) as freq_item
    from(
        select *,
            count(product_id) as most_frequent
        from Orders
        group by customer_id,product_id
        order by customer_id
    )tab
)tab2
where most_frequent=freq_item
)
select customer_id,f.product_id,p.product_name
from frequent_orders f join Products p
