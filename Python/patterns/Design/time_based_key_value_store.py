...
class TimeMap:
    ...
    def __init__(self):
        self.key_val_store = {} # Dictionary to store key-value pairs with timestamps

    def set(self, key: str, value: str, timestamp: int) -> None:
        if key not in self.key_val_store:
            self.key_val_store[key] = []
        # Append the value along with its timestamp as a list of value and timestamp
        self.key_val_store[key].append([value, timestamp])

    def get(self, key: str, timestamp: int) -> str:
        res = ""
        if key in self.key_val_store:
            values = self.key_val_store[key]
